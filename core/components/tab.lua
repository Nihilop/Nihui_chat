...
local function chatTab_SetPoint(self, point, anchor, relativePoint, x, y, shouldIgnore)
	if anchor == GeneralDockManager.scrollFrame.child and not shouldIgnore then
		self:ClearAllPoints()
		-- Add horizontal spacing between tabs
		local tabIndex = self:GetID()
		local spacing = (tabIndex - 1) * 5  -- 10px spacing between each tab
		self:SetPoint("BOTTOMLEFT", anchor, "BOTTOMLEFT", spacing, 0, true)
	end
end
...
local function chatTabText_SetPoint(self, p, anchor, rP, x, y, shouldIgnore)
	if not shouldIgnore then
		-- Always center vertically, add horizontal padding
		if p == "LEFT" then
			self:ClearAllPoints()
			self:SetPoint("LEFT", anchor, "LEFT", 20, 0, true)
		elseif p == "CENTER" then
			self:ClearAllPoints()
			self:SetPoint("CENTER", anchor, "CENTER", 0, 0, true)
		end
	end
end
...
function E:HandleChatTab(frame)
	...
	if C.db.profile.dock.font then
		local fontFace = frame.Text:GetFont()
		frame.Text:SetFont(fontFace, C.db.profile.dock.font.size, C.db.profile.dock.font.outline and "OUTLINE" or "")
	end
